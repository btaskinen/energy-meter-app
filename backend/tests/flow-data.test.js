...
describe('testing fetching data from server', () => {
  test('latest data is returned as JSON', async () => {
    const loginData = {
      username: 'JDoe',
      password: 'secret',
    };
  ...
  test('fetching latest data without authentication', async () => {
  ...
  test('all data is returned as JSON', async () => {
    const loginData = {
      username: 'JDoe',
      password: 'secret',
    };
  ...
  test('fetching latest data without authentication', async () => {
    const response = await api.get('/api/flow-data').expect(400);
    const returnedMessage = JSON.parse(response.text);
    expect(returnedMessage.error).toBe('jwt must be provided');
  });
});
